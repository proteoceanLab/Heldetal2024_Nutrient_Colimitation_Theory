...
def main():
     ...
     growth_rate_models = ["liebig_monod", "pat", "additive", "multiplicative_monod"]
     model_labels = [r"\textbf{Liebig model}", r"\textbf{Poisson arrival time}" + "\n" + r"\textbf{(PAT) model}", r"\textbf{Additive model}", r"\textbf{Multiplicative}" + "\n" + r"\textbf{Monod model}"]
     ...
     for m in range(len(growth_rate_models)):
          ...
          contourfs = axis.contourf(R1_mesh, R2_mesh, g_mesh, cmap="viridis", levels=growth_rate_levels)
          ...
          contours = axis.contourf(R1_mesh, R2_mesh, L1_mesh, levels=limitation_levels, alpha=0.5, colors=limitation_color_levels_1) #cmap="Blues") #colors="blue", 
          contours = axis.contourf(R1_mesh, R2_mesh, L2_mesh, levels=limitation_levels, alpha=0.5, colors=limitation_color_levels_2) #cmap="Oranges") #colors="orange", 
          ...
          if model == "liebig_monod":
               ...
               axis.plot([0, Ks[0]], [0, Ks[1]], "-", color=colormap.to_rgba(2))
               axis.plot([Ks[0], Ks[0]], [Ks[1], max(R2_range)], "-", color=colormap.to_rgba(2))
               axis.plot([Ks[0], max(R1_range)], [Ks[1], Ks[1]], "-", color=colormap.to_rgba(2))
               axis.scatter(Ks[0], Ks[1], color=colormap.to_rgba(3), s=2, marker="s", zorder=10)
