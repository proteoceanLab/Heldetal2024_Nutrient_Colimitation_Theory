...
def main():
     ...
     growth_rate_models = ["liebig_monod", "multiplicative_monod", "pat", "additive"]
     model_labels = [r"\textbf{Liebig Monod}" + "\n" + r"\textbf{model}", r"\textbf{Multiplicative}" + "\n" + r"\textbf{Monod model}", r"\textbf{Poisson arrival time}" + "\n" + r"\textbf{(PAT) model}", r"\textbf{Additive model}"]

     # Initialize contour levels for growth rate
     growth_rate_levels = numpy.linspace(0, 10, 11)/(numpy.linspace(0, 10, 11) + 1)

     # Initial contour levels for limitation coefficients
     limitation_levels = [0.125, 0.25, 0.5, 1]
     ...
     for m in range(len(growth_rate_models)):
          ...
          for i in range(len(R2_range_small)):
               for j in range(len(R1_range_small)):
                    ...
                    # Calculate Meff
                    Meff_mesh[i][j] = colimitation_models.CalcMeff([R1, R2], [a1, a2], gmax, model)

          # Initial growth rate landscape
          axis = figure.add_subplot(3, 4, m + 1)
          contourfs = axis.contourf(R1_mesh_large, R2_mesh_large, g_mesh, cmap="viridis", levels=growth_rate_levels)
          ...
          axis = figure.add_subplot(3, 4, m + 1 + len(growth_rate_models))
          contours = axis.contourf(R1_mesh_large, R2_mesh_large, L1_mesh, levels=limitation_levels, alpha=0.5, colors=limitation_color_levels_1) #cmap="Blues") #colors="blue", 
          contours = axis.contourf(R1_mesh_large, R2_mesh_large, L2_mesh, levels=limitation_levels, alpha=0.5, colors=limitation_color_levels_2) #cmap="Oranges") #colors="orange", 
          ...
          axis.set_ylim([min(R2_range_large), max(R2_range_large)])
          axis.set_xticks(numpy.linspace(0, max(R1_range_large), 6))
          axis.set_yticks(numpy.linspace(0, max(R2_range_large), 6))
          axis.text(7.5, 0.5, "$L^\mathrm{rate}_2 > 0.5$", fontsize=colimitation_plots.axis_label_size, ha="center", va="center")
          axis.text(7.5, 2, "$L^\mathrm{rate}_2 > 0.25$", fontsize=colimitation_plots.axis_label_size, ha="center", va="center")
          axis.text(7.5, 4, "$L^\mathrm{rate}_2 > 0.125$", fontsize=colimitation_plots.axis_label_size, ha="center", va="center")
          axis.text(0.5, 7.5, "$L^\mathrm{rate}_1 > 0.5$", fontsize=colimitation_plots.axis_label_size, ha="center", va="center", rotation=90)
          axis.text(2, 7.5, "$L^\mathrm{rate}_1 > 0.25$", fontsize=colimitation_plots.axis_label_size, ha="center", va="center", rotation=90)
          axis.text(4, 7.5, "$L^\mathrm{rate}_1 > 0.125$", fontsize=colimitation_plots.axis_label_size, ha="center", va="center", rotation=90)

          # Landscape of effective number of resources
          axis = figure.add_subplot(3, 4, m + 1 + 2*len(growth_rate_models))
          ...
          if model == "liebig_monod":
               colormap = cm.ScalarMappable(norm=matplotlib.colors.Normalize(vmin=1, vmax=3), cmap=cm.plasma)
               axis.plot([0, gmax/a1], [0, gmax/a2], "-", color=colormap.to_rgba(2))
               axis.plot([gmax/a1, gmax/a1], [gmax/a2, max(R2_range_small)], "-", color=colormap.to_rgba(2))
               axis.plot([gmax/a1, max(R1_range_small)], [gmax/a2, gmax/a2], "-", color=colormap.to_rgba(2))
               axis.scatter(gmax/a1, gmax/a2, color=colormap.to_rgba(3), s=2, marker="s", zorder=10)
