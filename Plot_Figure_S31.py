...
df = pd.read_excel("Data/Dataset_S3.xlsx", skiprows=11)
#df = df.iloc[:-3,]
df2 = pd.DataFrame()
...
bins = np.histogram(np.hstack((df['Meff_liebig'], df['Meff_PAT'])), bins=5)[1]
ax[0,0].hist(df['Meff_liebig'], bins, color='#0504aa', align='left', rwidth=0.95)
ax[1,0].hist(df['Meff_PAT'], bins, color='#0504aa', align='left',rwidth=0.95)
ax[2,0].hist(df['Meff_additive'], bins, color='#0504aa', align='left',rwidth=0.95)
ax[3,0].hist(df['Meff_multiplicative'], bins, color='#0504aa', align='left', rwidth=0.95)
...
bins = np.histogram(np.hstack((df2['Liebig_Monod'], df2['PAT'])), bins=5)[1]
ax[0,1].hist(df2['Liebig_Monod'], bins=bins, color='#0504aa', align='left', rwidth=0.95)
ax[1,1].hist(df2['PAT'], bins=bins, color='#0504aa', align='left',rwidth=0.95)
ax[2,1].hist(df2['Additive'], bins=bins, color='#0504aa', align='left',rwidth=0.95)
ax[3,1].hist(df2['Multiplicative_Monod'], bins=bins, color='#0504aa', align='left',rwidth=0.95)
