...
def main():
     """
     This script generates bootstrapped rate data sets from the original 
     experimental data set and then fits them to all rate models.
     """
     ...
     bootstraps_dataframe = colimitation_data_analysis.Generate2DScanBootstraps(R1_mesh, R2_mesh, rate_mesh, R1_label, R2_label, rate_label_base, num_bootstraps, seed=seed)
     bootstraps_dataframe.to_csv("./Data/Rate_bootstraps.csv", index=False)

     # Read back bootstrapped data from file
     rate_labels = [f"{rate_label_base} bootstrap {b + 1}" for b in range(num_bootstraps)]
     R1_mesh_bootstrap, R2_mesh_bootstrap, rate_mesh_bootstrap = colimitation_data_analysis.Read2DScan("./Data/Rate_bootstraps.csv", R1_label, R2_label, rate_labels, num_R1_values, num_R2_values) 
     ...
     for b in range(num_bootstraps):
          ...
          # Fit this bootstrap
          fit_dataframe = colimitation_data_analysis.FitAllModels(R1_mesh_bootstrap, R2_mesh_bootstrap, rate_mesh_bootstrap, [b], colimitation_models.all_2D_trait_models)
     ...
     fit_dataframe_combined.to_csv("./Data/Rate_bootstraps_fits.csv", index=False)
