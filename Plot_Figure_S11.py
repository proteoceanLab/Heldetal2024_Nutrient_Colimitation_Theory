...
def main():
     ...
     models_to_plot = ["liebig_blackman_rmin", "liebig_monod_rmin", "pat_rmin"]
...
################################################################################
     ...
     for m in range(len(models_to_plot)):
          ...
          axis.set_ylabel("Growth rate (per hour)", fontsize=colimitation_plots.axis_label_size)
          ...
          for i in range(len(R2_mesh_data.T)):
               for r in range(num_replicates):
                    xys = numpy.array([[R2_mesh_data[j][i], rate_mesh_data[j, i, r]] for j in range(len(R2_mesh_data[:, i])) if not isinstance(rate_mesh_data[j, i, r], str)])
                    if len(xys) != 0:
               ...
               rates_model_fit = colimitation_models.CalcTraitForFit((R1_mesh_data.T[i][0], R2s_model_fit), params, model_to_plot)
               axis.plot(R2s_model_fit, rates_model_fit, "-", color=glucose_colors[i])
          axis.set_xscale("symlog", linthresh=R2_mesh_data.T[0][2])

################################################################################

     figure.savefig("Figure_S11.pdf", bbox_inches="tight")
