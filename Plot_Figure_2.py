...
def main():
     ...
     rate_mesh_model = colimitation_models.CalcTraitForFit((R1_mesh_model, R2_mesh_model), params, model_to_plot)
     ...
     model_lim_coeffs_no_colim = "liebig_monod_rmin"
     model_lim_coeffs_with_colim = "pat_rmin"
     dataframe = pandas.read_csv(f"./Data/Rate_sim_{model_lim_coeffs_no_colim}_colim_fracs.csv", index_col=False)
     fracs_rel_colim_sims_no_colim = list(dataframe["Fraction of virtual supplementations with L1 and L2 > 0.0"])
     dataframe = pandas.read_csv(f"./Data/Rate_sim_{model_lim_coeffs_with_colim}_colim_fracs.csv", index_col=False)
...
################################################################################
     ...
     # Plot growth rate 2D scan data and fitted model
     axis = figure.add_subplot(gs[1, 0], projection="3d")
     axis.set_xlabel("Added glucose\nconcentration (mM)", fontsize=1.2*colimitation_plots.axis_label_size, labelpad=8)
     axis.set_ylabel("Added ammonium\nconcentration (mM)", fontsize=1.2*colimitation_plots.axis_label_size, labelpad=8)
     axis.set_zlabel("Growth rate\n(per hour)", fontsize=1.2*colimitation_plots.axis_label_size)
     axis.view_init(30, -135)
     axis.scatter(R1R2_list_data[0], R1R2_list_data[1], rate_list_data, color="tab:red", label="Data")
     axis.plot_surface(R1_mesh_model, R2_mesh_model, rate_mesh_model, label="Model fit")
...
################################################################################
     ...
     axis.set_ylabel("Growth rate (per hour)", fontsize=1.2*colimitation_plots.axis_label_size)
     ...
     for i in range(len(R1_mesh_data)):
          ...
          for r in range(num_replicates):
               xys = numpy.array([[R1_mesh_data[i][j], rate_mesh_data[i, j, r]] for j in range(len(R1_mesh_data[i])) if not isinstance(rate_mesh_data[i, j, r], str)])
               if len(xys) != 0:
          ...
          rates_model_fit = colimitation_models.CalcTraitForFit((R1s_model_fit, R2_mesh_data[i][0]), params, model_to_plot)
          axis.plot(R1s_model_fit, rates_model_fit, "-", color=ammonium_colors[i])
...
################################################################################
     ...
     axis.set_xlabel("Growth rate limitation coefficient\nfor glucose $L_\mathrm{glu}^\mathrm{rate}$", fontsize=1.2*colimitation_plots.axis_label_size)
     axis.set_ylabel("Growth rate limitation\ncoefficient for ammonium $L_\mathrm{amm}^\mathrm{rate}$", fontsize=1.2*colimitation_plots.axis_label_size, labelpad=0)
     for r in range(num_replicates):
          axis.scatter(lim_data[r]["L1"], lim_data[r]["L2"], s=10, label="Rep " + str(r + 1), color=rep_colors[r])
          axis.text(0.02, 0.98 - 0.08*r, r"\textbf{Rep " + str(r + 1) + "}", color=rep_colors[r], transform=axis.transAxes, ha="left", va="top", fontsize=1.2*colimitation_plots.axis_label_size)
     axis.set_xlim([-2, 2])
     axis.set_ylim([-0.5, 1])
     axis.fill_between([0, 2], [0, 0], y2=1, color="0.9", zorder=-1)
     axis.text(1, 0.95, r"\textbf{Colimitation}", fontsize=1.2*colimitation_plots.axis_label_size, ha="center", va="top")
...
################################################################################
     ...
     axis.set_xticks(numpy.linspace(*axis.get_xlim(), 5))
...
################################################################################
     ...
     figure.savefig("Figure_2.pdf", bbox_inches="tight")
