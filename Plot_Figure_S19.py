...
def main():
     ...
     dataframe = pandas.read_csv(f"./Data/Rate_sim_{null_model}_colim_fracs.csv", index_col=False)
...
################################################################################
     ...
     axis.set_xlabel("Threshold rate limitation coefficient $L^\mathrm{rate}$", fontsize=colimitation_plots.axis_label_size)
     axis.set_ylabel("Number of virtual supplementations with\n$L^\mathrm{rate}_\mathrm{glucose}$ and $L^\mathrm{rate}_\mathrm{ammonium}$ above threshold", fontsize=colimitation_plots.axis_label_size)
     ...
     figure.savefig("Figure_S19.pdf", bbox_inches="tight")
